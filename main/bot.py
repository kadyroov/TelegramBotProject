...
from main.models import (TelegramUser, TelegramUserIncome, 
                         TelegramAnswers, TelegramSupport, TelegramExpense)
...
def start(message):
    ...
    bot.send_message(message.chat.id, f"Добро пожаловать, {model_user.get_name()}!", reply_markup=menu_button())
...
def income_amount(message, name):
    ...
    income = TelegramUserIncome.objects.create(
        user=model_user,
        name=name, 
        amount=amount
    )
...
def income_source(message):
    if message.text.lower() == "отмена":
        ...
        cancel_button = types.ReplyKeyboardMarkup(resize_keyboard=True).add(types.KeyboardButton(text="Отмена"))
...
def chart_source(message):
    if message.text.lower() == "отмена":
        ...
        if r.status_code == 200:
            ...
            bot.send_message(message.chat.id, "График поврежден!", reply_markup=menu_button())
...
def run_bot():
    try:
        ...
        logger = logging.getLogger(f"Был запущен бот - {bot_username}.")
        print(f"Был запущен бот - {bot_username}.")
        bot.polling(none_stop=True, interval=0)
    except KeyboardInterrupt:
        logger.info("Бот был остановлен принудительно!")

    except SystemExit:
        logger.info("Бот остановлен, ошибка системы")
