...
class TelegramUserIncome(models.Model):
    ...
    name = models.CharField(
        ...
        help_text = "Откуда пришли деньги",
    )
    ...
    class Meta:
        ...
        verbose_name_plural = "Доходы телеграм"
...
class TelegramExpense(models.Model):
    ...
    name=models.CharField(
        ...
        help_text = "Расход куда потратились деньги"
    )
    amount=models.BigIntegerField(
        default=0,
        verbose_name="Сумма",
    ...
    created_at = models.DateTimeField(
        auto_now_add=True,
        verbose_name="Создан",
        help_text="Временная метка, указывающая, когда был расход."
    )
    def __str__(self) -> str:
        return f"{self.user.get_name()} - {self.amount}₸ ({self.name})"
    
    class Meta:
        verbose_name= "Расход телеграм"
        verbose_name_plural = "Расходы телеграм"
        ordering = ['-created_at']
